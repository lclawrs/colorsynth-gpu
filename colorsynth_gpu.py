...
def var_phase_shifting_5358(zLIN, zSIN, t=0.0):
    # use t for animation!
    phase_shift = cp.sin(t) * 0.5 + 0.5
    z = zLIN * cp.exp(1j * cp.angle(zSIN) * phase_shift)
    return z

def var_phase_shifting_1352(zLIN, zSIN, t=0.0):
    # use t for animation!
    phase = cp.sin(t)
    z = zLIN * cp.exp(1j * phase)
    return z

def var_phase_shifting_attractors_2353(zLIN, zSIN, t=0.0):
    # use t for animation!
    phase_shift = cp.sin(t)
    return (zLIN + zSIN * cp.exp(cp.complex(0, phase_shift)))

def var_phase_shifting_attractors_2414(zLIN, zSIN, t=0.0):
    # use t for animation!
    angle = cp.angle(zSIN) + t
    return cp.exp(1j * angle)

def var_phase_shifting_attractor_4355(zLIN, zSIN, t=0.0):
    # Phase-shifting the attractor patterns
    phase_shift = cp.sin(t) * 0.5 + 0.5
...
def var_phase_shifting_0355(zLIN, zSIN, t=0.0):
    # use t for animation!
    phase = cp.sin(t)
    return zLIN * cp.exp(cp.complex(0, phase))
...
VARIATIONS = [
    ...
    ("phase_shifting_5358", var_phase_shifting_5358),
    ("phase_shifting_1352", var_phase_shifting_1352),
    ...
    ("phase_shifting_colors", var_phase_shifting_colors),
    ("phase_shifting_0355", var_phase_shifting_0355),
]
...
COLORMAPS = {
    ...
    "phase_shifting_0353":    cmap_phase_shifting_0353,
    "phase_shifting_3354":    cmap_phase_shifting_3354,
}
